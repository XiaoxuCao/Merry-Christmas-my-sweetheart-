...
const SceneController: React.FC<{ 
    ...
    useFrame((state, delta) => {
        ...
        currentInput.current.x = THREE.MathUtils.lerp(currentInput.current.x, inputRef.current.x, inputSmoothing);
        currentInput.current.y = THREE.MathUtils.lerp(currentInput.current.y, inputRef.current.y, inputSmoothing);
        const camZ = zoomTarget.current + Math.abs(currentInput.current.x) * 2; 
        camera.position.lerp(vec.set(currentInput.current.x * 4, currentInput.current.y * 2, camZ), 4.0 * safeDelta);
        camera.lookAt(0, 0, 0);
        ...
        if (groupRef.current) {
            if (!!inputRef.current.isDetected) {
                ...
                groupRef.current.rotation.y = THREE.MathUtils.lerp(prevRot, targetAngle, 6.0 * safeDelta);
...
const SceneContent: React.FC<ExperienceProps> = ({ mixFactor, colors, inputRef, userImages, signatureText }) => {
  ...
  // 这里写死你的仓库名，绝对没错
  const repoName = 'Merry-Christmas-my-sweetheart-'; 
  ...
  return (
    <>
      ...
      {/* 修正点 1：删掉 public，加入前置斜杠和仓库名 */}
      <Environment 
        files={`/${repoName}/hdri/potsdamer_platz_1k.hdr`} 
        background={false} 
      />
...
const Experience: React.FC<ExperienceProps> = (props) => {
  return (
    <Canvas
      ...
      shadows
      style={{ touchAction: 'none', width: '100vw', height: '100vh', background: '#010a05' }}
    >
      {/* 修正点 2：必须加 Suspense，否则 3D 引擎加载资源时会由于异步问题直接报 reading S 崩溃 */}
